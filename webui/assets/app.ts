...
function focusSibling(row: HTMLElement, dir: 1 | -1): void {
	const rows = Array.from(document.querySelectorAll<HTMLElement>('.msg-row'))
...
function displayName(a?: Addr): string {
	if (!a) return '(unknown)'
...
function mailboxIcon(role: string | null, name: string): string {
	const r = role || name.toLowerCase()
	if (r.includes('inbox')) return '⬇'
	if (r.includes('sent')) return '➤'
	if (r.includes('draft')) return '✎'
	if (r.includes('trash')) return '🗑'
	if (r.includes('junk')) return '⚠'
	if (r.includes('archive')) return '🗄'
	return '🗀'
}
...
async function doLogin(): Promise<void> {
	...
	if (!userEmail || !pass) { status.textContent = 'enter email and password'; return }
...
function signOut(): void {
	authHeader = ''; accountId = ''; currentMailbox = ''; currentEmailId = ''
	inp('pass').value = ''
...
async function loadMailboxes(): Promise<void> {
	...
	if (inbox) await selectMailbox(inbox.id)
}
...
function renderNav(): void {
	...
	for (const m of mailboxes) {
		...
		;(el.querySelector('.lbl') as HTMLElement).textContent = m.name
		el.tabIndex = 0
		el.setAttribute('role', 'tab')
		el.setAttribute('aria-selected', m.id === currentMailbox ? 'true' : 'false')
		const go = () => { selectMailbox(m.id).catch(showListError) }
		el.onclick = go
		el.onkeydown = (e: KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); go() } }
		nav.appendChild(el)
	}
}

async function selectMailbox(id: string): Promise<void> {
	currentMailbox = id
	currentEmailId = ''
	renderNav()
...
function renderRow(em: Email, i: number): HTMLElement {
	...
	top.className = 'top'
	const fromEl = document.createElement('span'); fromEl.className = 'from'; fromEl.textContent = fromLabel
	const timeEl = document.createElement('span'); timeEl.className = 'time'; timeEl.textContent = relTime(em.receivedAt)
	top.appendChild(fromEl)
	if (junk) { const t = document.createElement('span'); t.className = 'tag-junk'; t.textContent = 'junk'; top.appendChild(t) }
	top.appendChild(timeEl)
	const subj = document.createElement('div'); subj.className = 'subj'; subj.textContent = em.subject || '(no subject)'
	const prev = document.createElement('div'); prev.className = 'prev'; prev.textContent = em.preview || ''
	content.appendChild(top); content.appendChild(subj); content.appendChild(prev)
...
function sanitizeHTML(raw: string): string {
	...
	doc.querySelectorAll('*').forEach(el => {
		for (const attr of Array.from(el.attributes)) {
			...
			if (name === 'href' && !safeLinkURL(val)) { el.removeAttribute(attr.name) }
			else if (name === 'src' && !safeImageURL(val)) { el.removeAttribute(attr.name) }
			else if (name === 'srcset') { el.removeAttribute(attr.name) } // avoid bypassing the src allow-list
		}
		if (el.tagName === 'A') { el.setAttribute('target', '_blank'); el.setAttribute('rel', 'noopener noreferrer nofollow') }
		if (el.tagName === 'IMG') { el.setAttribute('loading', 'lazy'); el.setAttribute('referrerpolicy', 'no-referrer') }
	})
...
function safeLinkURL(v: string): boolean {
	const s = v.toLowerCase()
	if (s.startsWith('http://') || s.startsWith('https://') || s.startsWith('mailto:') || s.startsWith('//')) return true
...
function safeImageURL(v: string): boolean {
...
function openCompose(prefillTo?: string): void {
	...
	inp('subject').value = ''
	;(inp('compose-text') as unknown as HTMLTextAreaElement).value = ''
	$('send-status').textContent = ''
	$('send-status').className = 'status'
	$('compose-scrim').classList.add('open')
	inp('to').focus()
}
function closeCompose(): void { $('compose-scrim').classList.remove('open') }

async function doSend(): Promise<void> {
	const to = inp('to').value.trim()
	const subject = inp('subject').value
	const text = (inp('compose-text') as unknown as HTMLTextAreaElement).value
	const from = userEmail
	const status = $('send-status')
	if (!to) { status.className = 'status'; status.textContent = 'recipient required'; return }
...
function wire(): void {
	...
	inp('pass').addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter') doLogin() })
	inp('user').addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter') inp('pass').focus() })
	...
	$('compose-scrim').addEventListener('click', e => { if (e.target === $('compose-scrim')) closeCompose() })
	document.addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Escape') closeCompose() })
}
